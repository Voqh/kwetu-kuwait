...
create table if not exists listings (
  ...
  -- Optional pin dropped by the poster. This is a soft "does this look like
...
returns table (
  ...
  edit_expires_at timestamptz, lat double precision, lng double precision
)
language plpgsql
security definer
...
begin
  ...
  insert into listings (area, block, type, description, rent_kwd, whatsapp_e164, lat, lng)
  values (
    btrim(p_area), nullif(btrim(p_block), ''), nullif(btrim(p_type), ''),
    nullif(btrim(p_description), ''), p_rent_kwd, p_whatsapp_e164, p_lat, p_lng
  )
  ...
  return query select
    ...
    now() + interval '2 minutes', new_listing.lat, new_listing.lng;
end;
$$;
...
begin
  ...
  return query select
    ...
    updated_listing.lat, updated_listing.lng;
end;
$$;
...
returns table (
  id uuid, area text, block text, type text, description text, rent_kwd numeric,
  whatsapp_e164 text, status text, created_at timestamptz, expires_at timestamptz,
  lat double precision, lng double precision, report_count integer
)
language plpgsql
security definer
set search_path = public
...
begin
  ...
  return query
    select l.id, l.area, l.block, l.type, l.description, l.rent_kwd, l.whatsapp_e164,
           l.status, l.created_at, l.expires_at, l.lat, l.lng, l.report_count
    from listings l
    where l.id = p_listing_id;
end;
$$;
...
revoke all on function create_public_listing(text, text, text, text, numeric, text, text, double precision, double precision) from public;
revoke all on function begin_public_listing_edit(uuid, text) from public;
revoke all on function update_public_listing(uuid, text, text, text, text, text, numeric, text, double precision, double precision) from public;
...
grant execute on function create_public_listing(text, text, text, text, numeric, text, text, double precision, double precision) to anon;
grant execute on function begin_public_listing_edit(uuid, text) to anon;
grant execute on function update_public_listing(uuid, text, text, text, text, text, numeric, text, double precision, double precision) to anon;
